...
function PatientDashboard({ user }) {
  ...
  const { data, loading, error, reload } = useAsyncData(
    async () => {
      const patient = await patientsService.getProfile().catch(() => null);
      const activePatientId = user?.patientId || patient?.id;

      const [metrics, appointments, prescriptions, reports, invoices, doctors] = await Promise.all([
        dashboardService.forRole("patient").catch(() => ({})),
        appointmentsService.listForPatient(activePatientId).catch(() => []),
        prescriptionsService.listByPatient(activePatientId).catch(() => []),
        reportsService.listByPatient(activePatientId).catch(() => []),
        billingService.listByPatient(activePatientId).catch(() => []),
      ...
      return {
        ...
        doctorsById: new Map((Array.isArray(doctors) ? doctors : []).map((doctor) => [doctor.id, doctor])),
      };
    },
    [user?.patientId],
  );
    ...
    <div className="page">
      ...
      <section className="section grid grid--split">
        <ChartContainer
          ...
          loading={loading}
        >
          {data?.metrics?.vitalsTrend?.systolic ? (
            <LineChart
            ...
            <EmptyState
              ...
              message="Blood pressure records will appear here as they are recorded."
            />
          )}
        </ChartContainer>
            ...
            {loading ? (
              ...
              <div className="col col--gap-md">
                <div className="row row--between">
                  <span className="stat__value">{data?.metrics?.adherence ?? 100}%</span>
                  <StatusBadge
                    kind="severity"
                    value={(data?.metrics?.adherence ?? 100) >= 80 ? "routine" : "urgent"}
                  />
                </div>
                <Progress
                  value={data?.metrics?.adherence ?? 100}
                  tone={(data?.metrics?.adherence ?? 100) >= 80 ? "success" : "warning"}
